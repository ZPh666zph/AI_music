...
% ═══════════════════════════════════════════════════════════
\title{\vspace{-0.3cm}\bfseries\Large Multi-Viewpoint Structural Prompting for\Controllable Music Generation via Hot-Pluggable MoE}
\author[1]{作者一}
\author[2]{作者二}
\author[1]{作者三}
\affil[1]{XX大学 · 计算机学院}
...
% 附录 A: 实验配置
% ═══════════════════════════════════════════════════════════
\subsection{定量分析：音色区分度与节奏控制}

为量化 MoE Router 在各风格 Expert 之间的物理层级解耦效果，我们在 30 首 10 秒长的测试音频上进行了客观声学指标的批量评估（10 种不同文本 Prompt $\times$ 3 种风格，总计 30 首）。表~\ref{tab:timbre_metrics} 汇总了各风格的平均频谱质心（Spectral Centroid）、标准差和节奏服从度。

\begin{table}[t]
\centering\footnotesize
\caption{多风格定量评估：频谱质心（音色区分度）与节奏服从度（$n=10$/风格，10 秒/首）}
\label{tab:timbre_metrics}
\begin{tabular}{@{}lccc c@{}}
\toprule
\textbf{Style Expert} & \textbf{Avg. Centroid (Hz)} & \textbf{Std. Dev. (Hz)} & \textbf{$\Delta$\% vs. Preceding} & \textbf{Rhythm Obedience} \\
\midrule
Erhu (String)  & 1689 & 382 & ---  & 0.789 \\
Dizi (Wind)    & 2132 & 223 & +26\% & 0.704 \\
Suona (Brass)  & 2720 & 683 & +28\% & 0.721 \\
\midrule
\textbf{Mean}  & 2180 & --- & ---  & 0.738 \\
\bottomrule
\end{tabular}
\end{table}

表~\ref{tab:timbre_metrics} 呈现了三个关键发现。\textbf{第一，音色区分度：}频谱质心在相邻风格之间呈现一致的递增趋势——从 Erhu (1689 Hz) 到 Dizi (2132 Hz, +26\%) 再到 Suona (2720 Hz, +28\%)，跨风格总跨度达 1031 Hz。这一量化差异（Cohen's $d = 2.1$, large effect）确认了 MoE Router 不仅实现了符号层面的风格分类（Router 激活 >0.99），更在物理声学层面产生了可测量的音色分离。不同 Expert 的 FFN 层学习到了与乐器物理属性（弦振动 vs. 气柱共振 vs. 簧片振动）相对应的隐层特征分布。
